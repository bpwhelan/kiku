...
Unlike most traditional note type, Kiku is entirely written in TypeScript using JavaScript UI library [SolidJS](https://www.solidjs.com/),
...
Kiku stores certain files in your Anki’s `collection.media` directory. All of these file names are prefixed with `_kiku`.
...
Some explanation about these files:
...
- `_kiku_db_similar_kanji_*.json.gz` are the database Kiku uses for similar kanji feature.
